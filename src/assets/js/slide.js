...
function moveTo(pageId) {
	$('.slide_div').css('-webkit-transform', 'translateX('+ (-100 * (+pageId)/8) +'%) translateZ(0)');
	// 重置 cur_page 的值
	cur_page = pageId;
  //添加过渡效果的class
...
function slide(options) {
    ...
    //当前滑动的页码，从0开始算
    // var cur_page= defaultObj.cur_page || 0;
    //保存touchstart的事件对象
    var touchFirst_obj;
    //保存touchend的事件对象
    var touchLast_obj;
    ...
    $(parent_wrap).on('touchstart', slide_page_wrap, function (e) {
        ...
        touchLast_obj = e.originalEvent.targetTouches[0];
        ...
        //touchend的回调函数
        defaultObj.endCallback && defaultObj.endCallback($(this), cur_page);
    });
}